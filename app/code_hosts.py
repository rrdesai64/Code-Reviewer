...
def review_body(scan: ScanResult, status: dict[str, Any], findings: list[dict[str, Any]]) -> str:
    ...
    total_priority_items = scan.summary.consolidated_findings or len(scan.findings)
    if total_priority_items > len(findings):
        lines.extend(['', f'Showing {len(findings)} of {total_priority_items} consolidated priority items. Raw findings remain in the full Secure Review artifact set.'])
