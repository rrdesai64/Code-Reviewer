...
from .models import BenchmarkLessonRequest, BenchmarkTransitionRequest, ChatNotificationRequest, CodeHostReviewRequest, DecisionRequest, DisposableVmScanRequest, FixApplyRequest, GatewaySendRequest, GitHubPrReviewRequest, HermesReviewRequest, HermesRunRequest, InsideOutAutofixLoopRequest, IssuePlanRequest, LLMRequest, MemoryRollbackRequest, QuarantineEntryRequest, QuarantineLookupRequest, RagMemoryReindexRequest, ReportLakeReindexRequest, RuntimeBuildRunRequest, RuntimeSmokeCheckRequest, TeachingLoopSessionRequest, TeamCampaignRequest, VerifiedAutofixRequest
...
from .runtime_smoke import run_runtime_smoke_checks, runtime_smoke_preview, runtime_smoke_status
...
from .teaching_loop import create_teaching_session, list_teaching_sessions, load_teaching_session, teaching_loop_status
...
@app.get('/api/runtime-smoke/status')
def runtime_smoke_status_endpoint(user: AuthUser = Depends(require_permission('enterprise:read'))) -> dict:
    status = runtime_smoke_status()
    audit(user.username, 'runtime_smoke.status_reported', 'runtime-smoke', {'phase': status['phase']})
    return status
...
@app.post('/api/scans')
async def create_scan(project_name: str | None = Form(None), repo_path: str | None = Form(None), coverage_path: str | None = Form(None), archive: UploadFile | None = File(None), user: AuthUser = Depends(require_permission('scan:run'))) -> dict:
    ...
    coverage_paths = [Path(coverage_path)] if coverage_path else None
    scan = run_scan(target, project_name=project_name, coverage_paths=coverage_paths)
    ...
    record_suppression_governance(scan, actor=user.username)
...
@app.get('/api/scans/{scan_id}/suppressions')
def scan_suppressions(scan_id: str, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = inline_suppression_report(scan)
    audit(user.username, 'suppressions.reported', scan_id, {'suppressed': str(report['suppressed_findings']), 'invalid': str(report['invalid_annotations'])})
    return report


@app.get('/api/scans/{scan_id}/consolidated-findings')
def scan_consolidated_findings(scan_id: str, limit: int | None = None, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = consolidated_findings_report(scan, limit=limit)
    audit(user.username, 'findings.consolidated_reported', scan_id, {'clusters': str(report['consolidated_findings']), 'cross_tool': str(report['cross_tool_clusters'])})
    return report


@app.get('/api/scans/{scan_id}/prioritization')
def scan_prioritization(scan_id: str, limit: int = 100, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = prioritization_report(scan, limit=limit)
    audit(user.username, 'findings.prioritization_reported', scan_id, {'active': str(report['summary']['active_findings']), 'top_score': str(report['summary']['top_score'])})
    return report


@app.get('/api/scans/{scan_id}/soundness')
def scan_soundness(scan_id: str, limit: int = 100, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
...
@app.get('/api/scans/{scan_id}/runtime-plan')
def scan_runtime_plan(scan_id: str, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = build_runtime_plan(scan)
    audit(user.username, 'runtime.plan_reported', scan_id, {
        'status': report['summary']['status'],
        'profiles': str(report['summary']['profile_count']),
...
@app.get('/api/scans/{scan_id}/runtime/smoke-preview')
def scan_runtime_smoke_preview(scan_id: str, profile_id: str | None = None, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    request = RuntimeSmokeCheckRequest(profile_id=profile_id)
    report = runtime_smoke_preview(scan, request)
    audit(user.username, 'runtime_smoke.preview_reported', scan_id, {
        'status': report['status'],
        'profile': report.get('selected_profile', {}).get('profile_id', ''),
    })
    return report


@app.post('/api/scans/{scan_id}/runtime/smoke-check')
def scan_runtime_smoke_check(scan_id: str, request: RuntimeSmokeCheckRequest, user: AuthUser = Depends(require_permission('scan:run'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = run_runtime_smoke_checks(scan, request)
    audit(user.username, 'runtime_smoke.check_reported', scan_id, {
        'status': report['status'],
        'network_probe': str(request.network_probe),
        'probe_count': str(report['summary']['probe_count']),
    })
    return report
...
def scan_reachability_context(scan_id: str, limit: int = 100, user: AuthUser = Depends(require_permission('scan:read'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = reachability_context_report(scan, limit=limit)
    audit(user.username, 'reachability.reported', scan_id, {'request_handlers': str(report['summary']['request_handler_findings']), 'changed_files': str(report['summary']['changed_file_findings'])})
    return report
...
@app.post('/api/scans/{scan_id}/fixes/verified-autofix')
def scan_verified_autofix(scan_id: str, request: VerifiedAutofixRequest, user: AuthUser = Depends(require_permission('fix:apply'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = run_verified_autofix(scan, request, actor=user.username)
    audit(user.username, 'fix.verified_autofix_requested', scan_id, {
        'status': report['status'],
        'gate': report['gate'],
