...
from .dast import dast_status, dast_verification_report
...
@app.get('/api/dast/status')
def dast_status_endpoint(user: AuthUser = Depends(require_permission('enterprise:read'))) -> dict:
    status = dast_status()
...
@app.post('/api/scans/{scan_id}/dast/verification')
def scan_dast_verification(scan_id: str, request: DastScanRequest, user: AuthUser = Depends(require_permission('scan:run'))) -> dict:
    try:
        scan = apply_decisions(load_scan(scan_id))
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail='scan not found')
    report = dast_verification_report(scan, request)
    audit(user.username, 'dast.verification_reported', scan_id, {
        'status': report['status'],
        'findings': str(report['summary']['dast_finding_count']),
        'blockers': str(report['gate']['blocking_issue_count']),
    })
    return report
