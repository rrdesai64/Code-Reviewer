...
SCHEMA_VERSION = 1
SUPPORTED_GATEWAY_CHANNELS = ('slack', 'teams', 'email', 'telegram')
...
def verify_shared_secret(channel: str, headers: dict[str, str], expected: str | None, header_names: tuple[str, ...] = ('x-secure-review-gateway-secret', 'x-secure-review-teams-secret')) -> None:
    allow_unsigned = parse_bool(os.getenv(f'GATEWAY_{channel.upper()}_ALLOW_UNSIGNED'), False)
    if not expected:
        if allow_unsigned:
            return
...
def channel_config(name: str) -> GatewayChannelConfig:
    ...
    enabled = (os.getenv(f'GATEWAY_{channel.upper()}_ENABLED') or 'auto').lower()
    dry_run = parse_bool(os.getenv(f'GATEWAY_{channel.upper()}_DRY_RUN'), parse_bool(os.getenv('GATEWAY_DRY_RUN'), True))
    ...
    return GatewayChannelConfig(
        ...
        dry_run=dry_run,
        outbound=True,
        inbound=channel in {'slack', 'teams', 'telegram', 'email'},
        required_env=required,
        missing_env=missing,
        target=channel_target(channel),
        allow_all_users=parse_bool(os.getenv(f'GATEWAY_{channel.upper()}_ALLOW_ALL_USERS'), parse_bool(os.getenv('GATEWAY_ALLOW_ALL_USERS'), False)),
...
def normalize_channel(channel: str) -> str:
    return str(channel or '').strip().lower().replace('_', '-')
...
def allowed_users(channel: str) -> set[str]:
    ...
    values.update(csv_env(os.getenv(f'GATEWAY_{channel.upper()}_ALLOWED_USERS')))
