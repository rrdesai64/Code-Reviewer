...
& .\.venv\Scripts\python.exe -m app.cli --path $Path --sarif-out $SarifOut --advanced-ai-out $AdvancedAiOut --cyclonedx-out $CycloneDxOut --spdx-out $SpdxOut --spdx-compliance-out $SpdxComplianceOut --sbom-policy-out $SbomPolicyOut --secret-policy-out $SecretPolicyOut --sbom-compare-out $SbomCompareOut --report-out $ReportOut --pr-comment-out pr-comment.md --compliance-out $ComplianceOut --fix-proposals-out $FixProposalsOut --remediation-plan-out $RemediationPlanOut
